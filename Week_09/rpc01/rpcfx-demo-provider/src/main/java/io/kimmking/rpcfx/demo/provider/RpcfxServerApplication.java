...
//		String userService = "io.kimking.rpcfx.demo.api.IUserService";
//		registerService(client, userService);
//		String orderService = "io.kimking.rpcfx.demo.api.IOrderService";
//		registerService(client, orderService);
	...
	/*private static void registerService(CuratorFramework client, String service) throws Exception {
		...
		try {
			final Stat stat = client.checkExists().forPath("/" + service);
			if ( null == stat) {
				client.create().withMode(CreateMode.EPHEMERAL).forPath("/" + service, "service".getBytes());
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	@Bean(name = "io.kimmking.rpcfx.demo.api.IUserService")
	public IUserService createUserService(){
		return new UserServiceImpl();
	}

	@Bean(name = "io.kimmking.rpcfx.demo.api.IOrderService")
